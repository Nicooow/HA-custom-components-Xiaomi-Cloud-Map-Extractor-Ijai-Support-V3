...
class XiaomiCloudMapExtractorConnector:
    ...
    def __init__(
        ...
        self._cloud_connector = XiaomiCloudConnector(
            session_creator, self._config.username, self._config.password
        )
    ...
    async def _initialize(self: Self) -> None:
        ...
        device_details = await self._cloud_connector.get_device_details(
            self._config.token, self._config.server
        )
    ...
    def _should_get_map(self: Self) -> bool:
        return (
            ...
            or self._vacuum_connector.should_update_map
        )
    ...
    def _create_device(
        ...
        store_map_path = (
            self._config.store_map_path if self._config.store_map_raw else None
        )
