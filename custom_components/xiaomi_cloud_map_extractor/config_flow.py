...
CLOUD_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional(CONF_SERVER, default="de"): vol.In(AVAILABLE_SERVERS),
    }
)
...
class XiaomiCloudMapExtractorFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    ) -> XiaomiCloudMapExtractorOptionsFlowHandler:
    ...
    async def async_step_cloud(
        ...
        if user_input is not None:
            ...
            connector = XiaomiCloudConnector(
                session_creator, username, password, server
            )
            two_factor_url = None
            try:
    ...
    async def async_step_select_vacuum(
        ...
        errors: dict[str, str] = {}
        if user_input is not None:
            self.cloud_vacuum = next(
                filter(
                    lambda v: v.device_id == user_input["select_vacuum"],
                    self.cloud_vacuums,
                )
            )
            return await self.async_step_confirm_data()
    ...
    async def async_step_confirm_data(
                    ...
                    return self.async_create_entry(
                        ...
                        options={
                            ...
                            CONF_DRAWABLES: [
                                e.value
                                for e in Drawable
                                if e != Drawable.ROOM_NAMES and "ignored" not in e
                            ],
        ...
        api_options: list[SelectOptionDict] = [
            SelectOptionDict(
                value=v, label=v.title() + (" *" if v == detected_api else "")
            )
            for v in VacuumApi
        ]
        confirm_data_schema = vol.Schema(
            {
                ...
                vol.Required(CONF_TOKEN, default=self.cloud_vacuum.token): vol.All(
        ...
        return self.async_show_form(
            ...
            data_schema=confirm_data_schema,
            errors=errors,
            last_step=True,
        )

    async def _validate_vacuum(
        self: Self, host: str, token: str, used_map_api: VacuumApi
    ) -> bool:
